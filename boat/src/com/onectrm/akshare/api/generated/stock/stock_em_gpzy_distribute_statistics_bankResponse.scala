...
import com.onectrm.akshare.api.generated.stock.stock_em_gpzy_distribute_statistics_bankResponse
...
/** 分类1: stock 接口: stock_em_gpzy_distribute_statistics_bank 目标地址:
  * http://data.eastmoney.com/gpzy/distributeStatistics.aspx 描述: 获取东方财富网-数据中心-特色数据-股权质押-质押机构分布统计-银行
  * 限量: 单次所有历史数据, 由于数据量比较大需要等待一定时间
  */
case class Respstock_em_gpzy_distribute_statistics_bankResponse(
...
case class stock_em_gpzy_distribute_statistics_bankResponse(
  `p_zhi4ya1gong1si1gu3piao4dai4ma3`: String,
  `p_zhi4ya1ji1gou4`: Double,
  `p_xing2ye4ming2cheng1`: Double,
  `p_zhi4ya1gong1si1shu4liang4`: Double,
...
trait Json_stock_em_gpzy_distribute_statistics_bankResponse {

  implicit val a_stock_em_gpzy_distribute_statistics_bankResponse =
    new Decoder[stock_em_gpzy_distribute_statistics_bankResponse] {
      final def apply(
        c: HCursor
      ): Decoder.Result[stock_em_gpzy_distribute_statistics_bankResponse] =
        for {
          ...
          stock_em_gpzy_distribute_statistics_bankResponse(
...
object JsonItem_stock_em_gpzy_distribute_statistics_bankResponse
    extends Json_stock_em_gpzy_distribute_statistics_bankResponse
