...
import com.onectrm.akshare.api.generated.stock.stock_zh_b_spotResponse
...
/** 分类1: stock 接口: stock_zh_b_spot 目标地址: http://vip.stock.finance.sina.com.cn/mkt/#hs_b 描述: B
  * 股数据是从新浪财经获取的数据, 重复运行本函数会被新浪暂时封 IP, 建议增加时间间隔 限量: 单次返回所有 B 股上市公司的实时行情数据
  */
case class Respstock_zh_b_spotResponse(
  `p_dai4ma3`: String,
...
case class stock_zh_b_spotResponse(
  `p_dai4ma3`: String,
  `p_ming2cheng1`: String,
  `p_zui4xin1jia4`: Double,
  `p_zhang3die1e2`: Double,
...
trait Json_stock_zh_b_spotResponse {

  implicit val a_stock_zh_b_spotResponse = new Decoder[stock_zh_b_spotResponse] {
    final def apply(c: HCursor): Decoder.Result[stock_zh_b_spotResponse] =
      for {
...
object JsonItem_stock_zh_b_spotResponse extends Json_stock_zh_b_spotResponse
