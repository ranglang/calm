...
import com.onectrm.akshare.api.generated.stock.stock_zh_a_spotResponse
...
/** 分类1: stock 接口: stock_zh_a_spot 目标地址: http://vip.stock.finance.sina.com.cn/mkt/#hs_a 描述: 新浪财经-沪深京
  * A 股数据, 重复运行本函数会被新浪暂时封 IP, 建议增加时间间隔 限量: 单次返回沪深京 A 股上市公司的实时行情数据
  */
case class Respstock_zh_a_spotResponse(
  `p_dai4ma3`: String,
...
case class stock_zh_a_spotResponse(
  `p_dai4ma3`: String,
  `p_ming2cheng1`: String,
  `p_zui4xin1jia4`: Double,
  `p_zhang3die1e2`: Double,
...
trait Json_stock_zh_a_spotResponse {

  implicit val a_stock_zh_a_spotResponse = new Decoder[stock_zh_a_spotResponse] {
    final def apply(c: HCursor): Decoder.Result[stock_zh_a_spotResponse] =
      for {
...
object JsonItem_stock_zh_a_spotResponse extends Json_stock_zh_a_spotResponse
