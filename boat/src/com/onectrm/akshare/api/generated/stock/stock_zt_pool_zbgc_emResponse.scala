...
import com.onectrm.akshare.api.generated.stock.stock_zt_pool_zbgc_emResponse
...
/** 分类1: stock 接口: stock_zt_pool_zbgc_em 目标地址: http://quote.eastmoney.com/ztb/detail#type=zbgc 描述:
  * 东方财富网-行情中心-涨停板行情-炸板股池 限量: 单次返回所有炸板股池数据
  */
case class Respstock_zt_pool_zbgc_emResponse(
  `p_xu4hao4`: Long,
  `p_dai4ma3`: String,
  ...
  `p_zhang3ting2tong3ji4`: Integer,
  `p_zhen4fu2`: String,
  `p_suo3shu3xing2ye4`: String
)

case class stock_zt_pool_zbgc_emResponse(
  `p_xu4hao4`: Long,
  `p_dai4ma3`: String,
  `p_ming2cheng1`: String,
  `p_zhang3die1fu2`: Double,
...
trait Json_stock_zt_pool_zbgc_emResponse {

  implicit val a_stock_zt_pool_zbgc_emResponse = new Decoder[stock_zt_pool_zbgc_emResponse] {
    final def apply(c: HCursor): Decoder.Result[stock_zt_pool_zbgc_emResponse] =
      for {
        ...
        p_zhang3su4 <- c.downField("涨速").as[Integer]

        p_shou3ci4feng1ban3shi2jian1 <- c.downField("首次封板时间").as[String]

        p_zha4ban3ci4shu4 <- c.downField("炸板次数").as[Integer]

        p_zhang3ting2tong3ji4 <- c.downField("涨停统计").as[Integer]

        p_zhen4fu2 <- c.downField("振幅").as[String]

        p_suo3shu3xing2ye4 <- c.downField("所属行业").as[String]

      } yield {
        stock_zt_pool_zbgc_emResponse(
          p_xu4hao4,
          p_dai4ma3,
          p_ming2cheng1,
          p_zhang3die1fu2,
...
object JsonItem_stock_zt_pool_zbgc_emResponse extends Json_stock_zt_pool_zbgc_emResponse
