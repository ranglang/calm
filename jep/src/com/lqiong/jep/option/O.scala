...
trait CaseClassFromMap1[P <: Product] {
  def apply(m: Map[String, Any]): Option[P]
}

object CaseClassFromMap1 {
  implicit def mk[P <: Product, R <: HList](implicit
    gen: LabelledGeneric.Aux[P, R],
    fromMap: FromMap[R]
  ): CaseClassFromMap1[P] = new CaseClassFromMap1[P] {
    def apply(m: Map[String, Any]): Option[P] = fromMap(m).map(gen.from)
  }

  def apply[P <: Product](map: Map[String, Any])(implicit fromMap: CaseClassFromMap1[P]): P =
    fromMap(map).get
}
