...
object JqSingleActor2 {
  sealed trait SingletonCommand

  case class SaveVix(replyTo: ActorRef[Either[Throwable, String]]) extends SingletonCommand

  case class SaveVix1(replyTo: ActorRef[Either[Throwable, String]]) extends SingletonCommand

  case class VixResponseWrapper(
    dateframe: String,
    ref: ActorRef[Either[Throwable, String]]
  ) extends SingletonCommand
  case class UFailure(
    eror: Throwable,
  ...
  def apply(
    ...
    def updated(isRunning: Boolean): Behavior[SingletonCommand] = {
