...
class CronTest extends FlatSpec with ShouldMatchers with BeforeAndAfterAll {
  ...
  "A.scala cron task" should "be able to be built fluently" in {
    val test = task {
  ...
  it should "be able to have a user defined description" in {
    ...
    val expected = "A.scala really cool description"
    val other = job {
      println("Blah blah")
  ...
  "A.scala Scheduled job" should "be able to handle errors optionally" in {
  ...
  "A.scala cron" should "be able to determine its next run" in {
  ...
  "A.scala cron" should "run even if it's already in time range" in {
